...
void Create::on_saveBtn_clicked()
{
    ...
    this->patient->lName = ui->lname->text();
...
//    header.setFontFamily("WenQuanYi Zen Hei");
    ...
    blockFormat.setBottomMargin(5);
    cursor.setBlockFormat(blockFormat);
    cursor.insertText("变应原点刺实验报告单",title);
    ...
    basicTableFormat.setWidth(printer->width()-40);
    basicTableFormat.setPadding(5);
    QTextTable *basicTable=cursor.insertTable(4,3, basicTableFormat);
    basicTable->mergeCells(2,0,1,3);
    basicTable->mergeCells(3,0,1,3);
    ...
    basicCursor.insertText("病案号：",tableTitle);
    basicCursor.insertText(this->patient->sBedNumber,tableContent);
    basicCursor=basicTable->cellAt(3,0).firstCursorPosition();
...
//    QTextFrame *symptomFrame = cursor.currentFrame();
    ...
    symptomTableFormat.setWidth(printer->width()-80);
    ...
    cursor.movePosition(QTextCursor::NextBlock,QTextCursor::MoveAnchor);
    cursor.insertBlock();
    cursor.insertText("实验结果", tableTitle);
    cursor.insertHtml("<hr />");
    ...
    leftTableFormat.setWidth(printer->width()-80);
    leftTableFormat.setAlignment(Qt::AlignCenter);
    leftTableFormat.setPadding(5);
    leftTableFormat.setCellPadding(12);
    ...
    rightTableFormat.setWidth(printer->width()-80);
    rightTableFormat.setAlignment(Qt::AlignCenter);
    rightTableFormat.setPadding(5);
    rightTableFormat.setCellPadding(12);
    QTextTable *rightTable=cursor.insertTable(4,6, rightTableFormat);
    rightTable->mergeCells(0,0,4,1);
    ...
    referCursor.insertText("阴性对照（生理盐水）",tableTitle);
...
//    tableTitle.setFontPointSize(8);
    ...
    cursor.insertText("备注:  "+this->patient->comment, tableTitle);
    cursor.insertBlock();
    ...
    cursor.insertBlock();
    cursor.insertText("注：耳鼻咽喉科免疫治疗室设门诊四楼（0731-89753265）");
    cursor.insertBlock();
    cursor.insertText("注：此报告单只供医师参考，不做证明使用。");
    cursor.insertBlock();
    tableTitle.setFontWeight(QFont::Black);
    cursor.insertText("检查医生:     "+this->patient->reporter, tableTitle);
    cursor.insertBlock();
    cursor.insertText("报告日期：    "+QDateTime::currentDateTime().toString("yyyy-MM-dd")) ;
